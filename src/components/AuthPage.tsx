...
export function AuthPage() {
  ...
  function openEmail(modeToOpen: 'signin' | 'signup' = 'signin') { setMode(modeToOpen); setEmailMode(true); setError(null); }
  function closeEmail() { setEmailMode(false); setError(null); setEmail(''); setPassword(''); }
  ...
  async function handleOAuthSignIn(provider: 'google' | 'apple') {
    ...
    try {
      ...
      if (Capacitor.isNativePlatform() && data?.url) {
        const finishedListener = await Browser.addListener('browserFinished', () => { setLoading(false); finishedListener.remove(); });
        await Browser.open({ url: data.url, presentationStyle: 'popover' });
      }
    } catch (err) { setError(err instanceof Error ? err.message : `${provider === 'google' ? 'Google' : 'Apple'} sign-in failed. Please try again.`); setLoading(false); }
  }
    ...
    <div className="min-h-screen bg-vow-bg flex flex-col items-center justify-center px-6 py-12">
      <div className="w-full max-w-sm">
        ...
        {!emailMode ? (
          ...
          <div>
            <button onClick={closeEmail} className="text-sm text-vow-muted hover:text-vow-ink mb-6 flex items-center gap-1 transition-colors"><ChevronLeft className="w-4 h-4" />Back</button>
            <div className="flex border border-vow-border mb-8">
              <button onClick={() => setMode('signin')} className={`flex-1 py-3 text-sm transition-colors ${mode === 'signin' ? 'bg-vow-ink text-vow-bg font-medium' : 'text-vow-muted hover:text-vow-ink'}`}>Sign in</button>
              <button onClick={() => setMode('signup')} className={`flex-1 py-3 text-sm transition-colors border-l border-vow-border ${mode === 'signup' ? 'bg-vow-ink text-vow-bg font-medium' : 'text-vow-muted hover:text-vow-ink'}`}>Create account</button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-5">
              <div><label className="vow-label block mb-2">Email</label><div className="relative"><Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-vow-muted" /><input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="vow-input pl-10" placeholder="you@example.com" autoFocus /></div></div>
