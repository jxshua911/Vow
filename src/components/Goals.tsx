...
export function GoalsPage() {
  ...
  const loadGoals = useCallback(async () => {
    ...
    if (requestedId && nextGoals.some((goal) => goal.id === requestedId)) {
      setSelectedGoalId(requestedId);
      localStorage.removeItem(`${OPEN_GOAL_PREFIX}${session.user.id}`);
    }
    setLoading(false);
  }, [session]);
...
function GoalDetail({ goalId, onBack }: { goalId: string; onBack: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    const [goalRes, msRes, sessRes] = await Promise.all([
      supabase.from('goals').select('*').eq('id', goalId).eq('user_id', session.user.id).maybeSingle(),
  ...
  function beginEditTime(sess: Session) { const date = new Date(sess.scheduled_at); setEditingSessionId(sess.id); setEditingTime(`${String(date.getHours()).padStart(2, '0')}:${String(date.getMinutes()).padStart(2, '0')}`); setSessionEditError(''); }
  async function saveSessionTime(sess: Session) {
    if (!session || !/^\d{2}:\d{2}$/.test(editingTime)) return;
    const [hours, minutes] = editingTime.split(':').map(Number);
    const nextDate = new Date(sess.scheduled_at); nextDate.setHours(hours, minutes, 0, 0);
    const { data: conflicts, error: conflictError } = await supabase.from('sessions').select('scheduled_at,duration_minutes').eq('user_id', session.user.id).eq('status', 'scheduled').neq('id', sess.id);
    if (conflictError) { setSessionEditError(conflictError.message); return; }
    const occupied = toOccupiedSlots((conflicts || []).map((row) => ({ scheduledAt: row.scheduled_at, durationMinutes: row.duration_minutes })));
    const exact = allocateSameDaySlot(nextDate, sess.duration_minutes, occupied);
    if (!exact || exact.date.getTime() !== nextDate.getTime()) { setSessionEditError('That time overlaps another commitment. Choose another time.'); return; }
    const { error } = await supabase.from('sessions').update({ scheduled_at: nextDate.toISOString(), updated_at: new Date().toISOString() }).eq('id', sess.id).eq('user_id', session.user.id);
    if (error) { setSessionEditError(error.message); return; }
    await supabase.from('goal_plan_items').update({ scheduled_at: nextDate.toISOString(), updated_at: new Date().toISOString() }).eq('goal_id', sess.goal_id).eq('user_id', session.user.id).eq('scheduled_at', sess.scheduled_at);
    setEditingSessionId(null); setEditingTime(''); setSessionEditError(''); await load();
  }
  ...
  return <div className="min-w-0 overflow-hidden">
    ...
    <div className="mb-10 min-w-0">
      <div className="border-t border-vow-border pt-8">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          {plan.baseline && <div className="border border-vow-border p-4 min-w-0"><p className="vow-label mb-2">Baseline</p><p className="text-sm text-vow-muted break-words">{plan.baseline}</p></div>}
        </div>
    ...
    <div className="mb-10"><div className="flex items-center justify-between mb-4"><h2 className="vow-label">Sessions & routine</h2></div><div className="border-t border-vow-border">{sessions.length === 0 ? <div className="py-6 text-sm text-vow-muted">No sessions scheduled for this goal yet.</div> : sessions.map((sess) => { const Icon = statusIcons[sess.status] || Circle; const editing = editingSessionId === sess.id; return <div key={sess.id} className="border-b border-vow-border py-4 min-w-0"><div className="flex items-center gap-4 min-w-0"><Icon className="w-4 h-4 text-vow-muted shrink-0" /><div className="flex-1 min-w-0 overflow-hidden"><p className="text-sm text-vow-ink break-words">{sess.title}</p>{!editing && <p className="text-xs text-vow-muted mt-1">{formatDate(sess.scheduled_at)} · {formatTime(sess.scheduled_at)} · {sess.duration_minutes} min</p>}{editing && <div className="mt-2 flex items-center gap-2 flex-wrap"><input type="time" value={editingTime} onChange={(e) => setEditingTime(e.target.value)} aria-label="New session time" className="vow-input w-auto min-w-[120px] py-2" autoFocus /><button onClick={() => saveSessionTime(sess)} disabled={!/^\d{2}:\d{2}$/.test(editingTime)} className="vow-btn-primary text-xs">Save time</button><button onClick={() => { setEditingSessionId(null); setEditingTime(''); setSessionEditError(''); }} className="vow-btn-ghost text-xs">Cancel</button></div>}</div>{sess.status === 'scheduled' && !editing && <div className="flex gap-2 shrink-0"><button onClick={() => beginEditTime(sess)} className="vow-btn-soft text-xs">Change time</button><button onClick={() => updateSessionStatus(sess.id, 'completed')} className="vow-btn-soft text-xs">Complete</button><button onClick={() => updateSessionStatus(sess.id, 'skipped')} className="vow-btn-soft text-xs">Skip</button><button onClick={() => moveSession(sess)} className="vow-btn-soft text-xs">Move</button></div>}</div>{editing && sessionEditError && <p className="text-xs text-vow-ink border-l-2 border-vow-ink pl-3 mt-3 ml-8 break-words">{sessionEditError}</p>}</div>; })}</div></div>
