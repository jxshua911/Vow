...
export function CalendarPage() {
  ...
  const loadGoogleEvents = useCallback(async () => {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : 'Could not load Google Calendar.'); }
  ...
  async function connectGoogleCalendar() {
    ...
    setError(null);
    try {
      const { data, error: functionError } = await supabase.functions.invoke('google-calendar-auth', { body: {} });
      if (functionError) throw functionError;
      if (!data?.authorization_url) throw new Error('Could not start Google Calendar authorization.');
