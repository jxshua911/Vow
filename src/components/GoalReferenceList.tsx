...
export function GoalReferenceList({ goalId }: { goalId: string }) {
  ...
  async function load() {
    const [{ data: goalData }, { data, error: resourceError }] = await Promise.all([
  ...
  return <section className="mb-10">
    <div className="flex items-start justify-between gap-4 mb-4"><div><h2 className="vow-label">Goal references</h2><p className="text-xs text-vow-muted mt-1">VOW ranks resources for this exact goal and plan context.</p></div><button onClick={discover} disabled={!goal || discovering} className="vow-btn-ghost text-xs shrink-0 flex items-center gap-1"><RefreshCw className={`w-3 h-3 ${discovering ? 'animate-spin' : ''}`} />{discovering ? 'Searching...' : resources.length ? 'Refresh resources' : 'Find resources'}</button></div>
