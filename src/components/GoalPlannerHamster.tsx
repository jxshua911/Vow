...
export function GoalPlanner({userId,onCreated,onCancel,draftGoal}:{userId:string;onCreated:()=>void;onCancel?:()=>void|Promise<void>;draftGoal?:DraftGoal|null}){
 const [goal,setGoal]=useState(draftGoal?.title||draftGoal?.outcome||''); const [why,setWhy]=useState(draftGoal?.why_it_matters||''); const [armadillo,setArmadillo]=useState<ArmadilloResult|null>(null); const [specialist,setSpecialist]=useState<ReturnType<typeof specialistFor>|null>(null);
 ...
 async function start(){
  ...
  try{
   ...
   const {data,error:e}=await supabase.functions.invoke('vow-hamster',{body:{action:'clarify',goal:{id,title:originalGoal,outcome:originalGoal,why_it_matters:why.trim()||null},armadillo:{...a,category:s.domain||a.category,goal_type:s.goal_types[0]||a.goal_type},hamster_context:{...context,mode:inferred,domain:s},answers:[],required_inputs:s.required_inputs||[]}});
   if(e||!data?.structured) throw new Error(USER_MESSAGES.clarification_unavailable);
   const qs:ClarificationQuestion[]=Array.isArray(data.structured.questions)?data.structured.questions.map((value:unknown)=>{if(!value||typeof value!=='object')return null;const item=value as Record<string,unknown>;return {key:typeof item.key==='string'?item.key.trim():'',question:typeof item.question==='string'?item.question.trim():''};}).filter((value: ClarificationQuestion | null): value is ClarificationQuestion=>Boolean(value?.key&&value.question)):[];
   if(s.required_inputs.length){
    ...
    if(missing.length)throw new Error(USER_MESSAGES.clarification_unavailable);
    if(qs.some(question=>!requiredKeys.has(question.key)))throw new Error(USER_MESSAGES.clarification_unavailable);
   }
 ...
 async function build(){
  ...
  try{
   ...
   if(clean.length!==questions.length||clean.some(answer=>!answer))throw new Error(USER_MESSAGES.invalid_request);
   ...
   const ctx=buildHamsterContext({title:goal,outcome:goal,why_it_matters:why,category:s.domain||a.category,goal_type:s.goal_types[0]||a.goal_type,planning_mode:mode});
   const request=supabase.functions.invoke('vow-hamster',{body:{action:'plan',goal:{id:draftId,title:goal,outcome:goal,why_it_matters:why.trim()||null},armadillo:{...a,category:s.domain||a.category,goal_type:s.goal_types[0]||a.goal_type},hamster_context:{...ctx,mode,domain:s},answers:questions.map((question,index)=>({key:question.key,question:question.question,answer:clean[index]})),required_inputs:s.required_inputs||[]}});
   const result=await Promise.race([request,new Promise<never>((_,reject)=>window.setTimeout(()=>reject(new Error(USER_MESSAGES.plan_unavailable)),30000))]);
   const {data,error:e}=result;
   if(e||!data?.structured)throw new Error(USER_MESSAGES.plan_unavailable);
   if(!mode)throw new Error(USER_MESSAGES.plan_unavailable);
   const next=normalisePlan(data.structured,mode);
   if(!next.steps.length)throw new Error(USER_MESSAGES.plan_unavailable);
   setPlan(next);setReadyToBuild(false);
 ...
 async function lock(){
  ...
  setBusy(true);setError('');
  try{
   const context={planning:{mode:plan.mode,workflow:HAMSTER_WORKFLOWS[plan.mode],specialist,plan},analysis:armadillo||{},personalisation:{questions,answers,completed:answers.length>0&&answers.every(Boolean)}};
   const {error:e}=await supabase.from('goals').update({title:goal,outcome:plan.completion_definition||goal,why_it_matters:why.trim()||null,status:'active',plan_json:plan,goal_context_json:context,plan_version:2,plan_generated_at:new Date().toISOString(),planning_horizon_weeks:null,planning_timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,deadline:null}).eq('id',draftId).eq('user_id',userId);
   if(e)throw e;
   await supabase.from('milestones').delete().eq('goal_id',draftId);
   if(plan.milestones?.length)await supabase.from('milestones').insert(plan.milestones.map((m,i)=>({goal_id:draftId,title:m.title,description:m.description,sort_order:i,status:i===0?'in_progress':'pending',deadline:null})));
   onCreated();
 ...
 if(questions.length)return <div><PageHeader title="VOW needs your context" subtitle={`${workflow?.name||'Personalised'} planning · ${specialist?.domain||'your domain'}`} /><div className="max-w-xl space-y-5"><p className="text-sm text-vow-muted">Every question is required. VOW uses each answer to shape the plan.</p>{questions.map((q,i)=><div key={`${q.key}-${i}`}><label className="vow-label block mb-2" htmlFor={`vow-q-${i}`}>{q.question} <span aria-hidden="true">*</span></label><textarea id={`vow-q-${i}`} rows={4} value={answers[i]||''} onChange={e=>setAnswers(x=>x.map((a,j)=>j===i?e.target.value:a))} className="vow-input resize-none" placeholder="Your answer..." required aria-required="true" /></div>)}{error&&<p className="text-sm text-vow-ink border-l-2 border-vow-ink pl-3">{error}</p>}<div className="flex gap-3"><button className="vow-btn-ghost" onClick={()=>setQuestions([])}>Back</button><button className="vow-btn-primary flex-1" onClick={build} disabled={busy||answers.length!==questions.length||answers.some(a=>!a.trim())}>{busy?'Building your plan...':'Build my VOW'}</button></div></div></div>;
 ...
 return <div><PageHeader title="Make your VOW" subtitle="Tell VOW what you want to accomplish. The system chooses the right domain specialist and planning workflow for you." /><div className="max-w-xl space-y-5"><textarea autoFocus rows={4} value={goal} onChange={e=>setGoal(e.target.value)} className="vow-input resize-none" placeholder="e.g. Make ravioli for my family on Saturday" /><textarea rows={3} value={why} onChange={e=>setWhy(e.target.value)} className="vow-input resize-none" placeholder="Why does this matter?" />{error&&<p className="text-sm text-vow-ink border-l-2 border-vow-ink pl-3">{error}</p>}<div className="flex gap-3"><button className="vow-btn-ghost" onClick={()=>void onCancel?.()}>Cancel</button><button className="vow-btn-primary flex-1" disabled={!goal.trim()||busy} onClick={start}>{busy?'VOW is understanding your goal...':'Continue'}</button></div></div></div>;
}
