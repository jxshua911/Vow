...
function safeHamsterMessage(value: unknown, fallback: string) {
 ...
 const internalCodes = [
  'HAMSTER_DISCOVERY_FAILED',
  'HAMSTER_EMPTY_PLAN',
  'VOW could not',
  'VOW requires',
  'Missing required clarification input',
  'Answer every VOW question',
  'VOW did not',
  'VOW returned'
 ];

 if (internalCodes.some(code => raw.includes(code))) {
  return fallback;
 }
...
export function GoalPlanner({userId,onCreated,onCancel,draftGoal}:{userId:string;onCreated:()=>void;onCancel?:()=>void|Promise<void>;draftGoal?:DraftGoal|null}){
 ...
 async function start(){
  ...
  try{
   ...
   const a=analyseGoalForEvidence({title:goal,outcome:goal,why_it_matters:why});
   const s=specialistFor({title:goal,category:a.category,goal_type:a.goal_type});
   setArmadillo(a);
   setSpecialist(s);

   const inferred=inferHamsterMode({title:goal,outcome:goal,why_it_matters:why,category:a.category,goal_type:a.goal_type,target:a.target,time_target:a.time_target});
   setMode(inferred);

   const context=buildHamsterContext({
    title:goal,
    outcome:goal,
    why_it_matters:why,
    category:s?.domain||a.category,
    goal_type:s?.goal_types[0]||a.goal_type,
    planning_mode:inferred
   });

   const id=await draft();

   const {data,error:e}=await supabase.functions.invoke('vow-hamster',{
    body:{
     action:'clarify',
     goal:{
      id,
     ...
     armadillo:{...a,category:s?.domain||a.category,goal_type:s?.goal_types[0]||a.goal_type},
     hamster_context:{...context,mode:inferred,domain:s},
     answers:[],
     required_inputs:s?.required_inputs||[]
    }
   });

   if(e||!data?.structured){
    throw new Error(safeHamsterMessage(
     data?.error||e,
     USER_MESSAGES.clarification_unavailable
    ));
   }

   const qs:ClarificationQuestion[]=Array.isArray(data.structured.questions)
    ?data.structured.questions.map((value:unknown)=>{
     ...
     }).filter((value: ClarificationQuestion | null): value is ClarificationQuestion=>Boolean(value?.key&&value.question))
       .slice(0,(s?.required_inputs?.length||0)+1)
    :[];

   if(s?.required_inputs?.length){
    const requiredKeys=new Set(s.required_inputs.map(input=>input.key));
    const returnedKeys=new Set(qs.map(question=>question.key));
    const missing=s.required_inputs.map(input=>input.key).filter(key=>!returnedKeys.has(key));

    if(missing.length){
     throw new Error(USER_MESSAGES.clarification_unavailable);
    }

    if(qs.some(question=>!requiredKeys.has(question.key))&&qs.length>s.required_inputs.length+1){
     throw new Error(USER_MESSAGES.clarification_unavailable);
    }
   }
   ...
   setQuestions(qs);
   setAnswers(qs.map(()=>'')); 
   setReadyToBuild(qs.length===0);

   await supabase.from('goal_clarification_answers').delete().eq('goal_id',id).eq('user_id',userId);

   if(qs.length){
    const {error:saveError}=await supabase.from('goal_clarification_answers').insert(
   ...
   setError(safeHamsterMessage(e,USER_MESSAGES.clarification_unavailable));
  }finally{
   setBusy(false);
  }
 }
 ...
 async function build(){
  ...
  try{
   ...
   if(clean.length!==questions.length||clean.some(answer=>!answer)){
    throw new Error(USER_MESSAGES.invalid_request);
   }

   await safe(clean.join('\n'));

   await supabase.from('goal_clarification_answers').delete().eq('goal_id',draftId).eq('user_id',userId);

   if(questions.length){
    const {error:answerError}=await supabase.from('goal_clarification_answers').insert(
   ...
   const a=armadillo||analyseGoalForEvidence({title:goal,outcome:goal,why_it_matters:why});
   const s=specialist||specialistFor({title:goal,category:a.category,goal_type:a.goal_type});

   const ctx=buildHamsterContext({
    title:goal,
    outcome:goal,
    why_it_matters:why,
    category:s?.domain||a.category,
   ...
   const request= supabase.functions.invoke('vow-hamster',{
    body:{
     action:'plan',
     goal:{
      id:draftId,
     ...
     armadillo:{...a,category:s?.domain||a.category,goal_type:s?.goal_types[0]||a.goal_type},
     hamster_context:{...ctx,mode,domain:s},
   ...
   const result=await Promise.race([
    ...
    new Promise<never>((_,reject)=>window.setTimeout(
     ()=>reject(new Error(USER_MESSAGES.plan_unavailable)),
     30000
    ))
   ]);
   ...
   if(e||!data?.structured){
    throw new Error(safeHamsterMessage(
     data?.error||e,
     USER_MESSAGES.plan_unavailable
    ));
   }

   if(!mode){
    throw new Error(USER_MESSAGES.plan_unavailable);
   }

   const next=normalisePlan(data.structured,mode);
   ...
   console.error('[VOW] Plan build failed:',e);
   setPlan(null);
   setError(safeHamsterMessage(e,USER_MESSAGES.plan_unavailable));
  }finally{
   setBusy(false);
  }
 }
 ...
 async function lock(){
  ...
  try{
   const context={
    planning:{
     mode:plan.mode,
     workflow:HAMSTER_WORKFLOWS[plan.mode],
     specialist,
     plan
    },
    analysis:armadillo||{},
    personalisation:{
     questions,
     answers,
     completed:answers.length>0&&answers.every(Boolean)
    }
   };

   const {error:e}=await supabase.from('goals').update({
    title:goal.trim(),
    outcome:plan.completion_definition||goal.trim(),
    why_it_matters:why.trim()||null,
    status:'active',
    plan_json:plan,
    goal_context_json:context,
    plan_version:2,
   ...
   await supabase.from('milestones').delete().eq('goal_id',draftId);
   ...
   setError(e instanceof Error?e.message:'Could not lock this VOW.');
  }finally{
   setBusy(false);
  }
 }
 ...
 if(plan){
  const planWorkflow=HAMSTER_WORKFLOWS[plan.mode]||HAMSTER_WORKFLOWS.adaptive;

  return <div>
   <PageHeader title="Your VOW" subtitle={`Personalised planning · ${plan.domain}`} />

   <div className="max-w-3xl space-y-5">
    <section className="border border-vow-border p-5">
     <p className="vow-label mb-2">How this goal works</p>
     <p className="text-lg font-medium text-vow-ink">{plan.overview}</p>
     <p className="text-sm text-vow-muted mt-3">{plan.completion_definition}</p>
    </section>

    <section className="border border-vow-border p-5">
     <p className="vow-label mb-3">Execution</p>
     <div className="space-y-4">
      {plan.steps.map((s)=>
       <div key={`${s.order}-${s.title}`} className="border-l-2 border-vow-ink pl-4">
        <p className="text-sm font-medium text-vow-ink">{s.order}. {s.title}</p>
        <p className="text-xs text-vow-muted mt-1">{s.purpose}</p>
    ...
    <section className="border border-vow-border p-5">
     <p className="vow-label mb-3">Planning</p>
     <p className="text-sm text-vow-ink">{planWorkflow.name}: {planWorkflow.purpose}</p>
     <p className="text-sm text-vow-muted mt-2">
      Horizon: {plan.horizon} · Cadence: {plan.cadence} · Deadline: {plan.deadline}
     </p>
     <p className="text-sm text-vow-muted mt-2">{plan.next_action}</p>
    </section>

    {plan.adaptation_rules?.length>0&&
     <section className="border border-vow-border p-5">
      <p className="vow-label mb-3">What changes the plan</p>
      {plan.adaptation_rules.map(rule=>
       <p key={rule} className="text-sm text-vow-muted py-1">{rule}</p>
      )}
     </section>
    }

    {error&&
     <p className="text-sm text-vow-ink border-l-2 border-vow-ink pl-3">{error}</p>
    }

    <div className="flex gap-3">
     <button className="vow-btn-ghost" onClick={()=>setPlan(null)}>Adjust</button>
     <button className="vow-btn-primary flex-1" onClick={lock} disabled={busy}>
      {busy?'Saving...':'Lock in VOW'}
     </button>
    </div>
   </div>
  </div>;
 ...
 if(questions.length){
  return <div>
   <PageHeader title="VOW needs your context" subtitle={`${workflow?.name||'Personalised'} planning · ${specialist?.domain||'your domain'}`} />

   <div className="max-w-xl space-y-5">
    <p className="text-sm text-vow-muted">Every question is required. VOW uses each answer to shape the plan.</p>

    {questions.map((q,i)=>
     <div key={`${q.key}-${i}`}>
      <label className="vow-label block mb-2" htmlFor={`vow-q-${i}`}>
       {q.question} <span aria-hidden="true">*</span>
      </label>
      ...
      <textarea
       ...
       rows={4}
       value={answers[i]||''}
       onChange={e=>setAnswers(x=>x.map((a,j)=>j===i?e.target.value:a))}
    ...
    <div className="flex gap-3">
     <button className="vow-btn-ghost" onClick={()=>setQuestions([])}>Back</button>
     <button
      className="vow-btn-primary flex-1"
      onClick={build}
      disabled={busy||answers.length!==questions.length||answers.some(a=>!a.trim())}
     >
      {busy?'Building your plan...':'Build my VOW'}
     </button>
    </div>
   </div>
  </div>;
 ...
 if(readyToBuild){
  return <div>
   <PageHeader title="Your goal is ready" subtitle={`${workflow?.name||'Personalised'} planning · ${specialist?.domain||'your domain'}`} />

   <div className="max-w-xl space-y-5">
    <p className="text-sm text-vow-muted">
     VOW has enough information to build this goal without additional questions.
    </p>
    ...
    <div className="flex gap-3">
     <button className="vow-btn-ghost" onClick={()=>{setReadyToBuild(false);setError('');}}>
      Back
     </button>
 ...
 return <div>
  <PageHeader title="Make your VOW" subtitle="Tell VOW what you want to accomplish. The system chooses the right domain specialist and planning workflow for you." />
  ...
  <div className="max-w-xl space-y-5">
   <textarea
    ...
    className="vow-input resize-none"
    placeholder="e.g. Make ravioli for my family on Saturday"
   />
   ...
   <div className="flex gap-3">
    <button className="vow-btn-ghost" onClick={()=>void onCancel?.()}>Cancel</button>
    ...
    <button
     ...
     onClick={start}
    >
     {busy?'VOW is understanding your goal...':'Continue'}
    </button>
   </div>
  </div>
 </div>;
