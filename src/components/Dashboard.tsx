  ...
  return <div>
    ...
    <div className="grid md:grid-cols-2 gap-12">
      <div>
        ...
        {upcoming.length === 0 ? <div className="border border-vow-border p-8 text-center"><Calendar className="w-7 h-7 text-vow-border mx-auto mb-3" strokeWidth={1} /><p className="text-vow-muted text-sm mb-3">No sessions scheduled.</p><button onClick={() => onNavigate('goals')} className="text-vow-ink text-sm font-medium border-b border-vow-ink pb-0.5 hover:opacity-70 transition-opacity">Schedule sessions</button></div> : <div className="space-y-px border border-vow-border">{upcoming.map((s) => { const StatusIcon = statusIcons[s.status] || Circle; const goal = goals.find((g) => g.id === s.goal_id); return <div key={s.id} className="bg-vow-bg px-4 py-3 flex items-center gap-3"><StatusIcon className="w-4 h-4 flex-shrink-0 text-vow-muted" strokeWidth={1.5} /><div className="flex-1 min-w-0"><div className="text-sm text-vow-ink truncate">{s.title}</div><div className="text-xs text-vow-muted">{goal?.title || ''}</div></div><div className="text-right flex-shrink-0"><div className="text-xs text-vow-ink font-medium">{formatRelative(s.scheduled_at)}</div><div className="text-xs text-vow-muted">{formatTime(s.scheduled_at)}</div></div></div>; })}</div>}
      </div>
      <div>
    ...
    <div className="mt-8 border-t border-vow-border pt-5"><p className="text-xs text-vow-muted">Your journal remains private and is available only from Journal.</p><button onClick={() => onNavigate('journal')} className="mt-2 text-vow-ink text-sm border-b border-vow-ink">Open Journal</button></div>
  </div>;
}
