...
function ConfirmedReviewView({ review, pastReviews, onDelete }: { review: Review; pastReviews: Review[]; onDelete: (id: string) => void }) { return <div className="min-w-0 overflow-hidden"><PageHeader title="Weekly Review" subtitle={`${formatDate(review.week_start)} — ${formatDate(review.week_end)}`} /><div className="border-l-2 border-vow-success pl-4 mb-10"><p className="text-sm text-vow-ink font-medium">Review confirmed</p><p className="text-xs text-vow-muted mt-0.5 break-words">Next week's commitments are locked in and sessions are scheduled.</p></div><RavenReviewSection review={review} /><ReviewContent review={review} /><div className="mt-8 pt-5 border-t border-vow-border"><button onClick={() => onDelete(review.id)} className="text-xs text-vow-muted hover:text-vow-ink">Delete this review</button></div>{pastReviews.length > 1 && <PastReviewsList reviews={pastReviews.slice(1)} onDelete={onDelete} />}</div>; }
