...
function ReviewContent({ review }: { review: Review }) {
  ...
  return (
    <div className="space-y-10">
      {/* Metrics */}
      <div className="grid grid-cols-4 gap-px bg-vow-border border border-vow-border">
        <MetricCell label="Committed" value={review.committed_count} />
        <MetricCell label="Completed" value={review.completed_count} />
        <MetricCell label="Missed" value={review.missed_count} />
        <MetricCell label="Moved" value={review.moved_count} />
      </div>
      ...
      <div className="flex items-center gap-8">
        <div className="relative w-20 h-20 flex-shrink-0">
          <svg className="w-20 h-20 -rotate-90" viewBox="0 0 100 100">
            <circle cx="50" cy="50" r="44" fill="none" stroke="#E2E2DF" strokeWidth="2" />
            <circle
              cx="50" cy="50" r="44" fill="none" stroke="#111111" strokeWidth="2" strokeLinecap="square"
              strokeDasharray={`${(review.completion_pct / 100) * 276.46} 276.46`}
              className="transition-all duration-1000"
            />
          </svg>
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-lg vow-heading text-vow-ink">{Math.round(review.completion_pct)}%</span>
          </div>
        </div>
        <div>
          <p className="vow-label mb-1">Completion rate</p>
          <p className="text-sm text-vow-ink">
            {review.committed_count > 0
              ? `You completed ${review.completed_count} of ${review.committed_count} sessions.`
      ...
      {/* Win / Setback */}
      <div className="grid md:grid-cols-2 gap-px bg-vow-border border border-vow-border">
        {review.biggest_win && (
          <div className="bg-vow-bg p-5">
            <p className="vow-label mb-2">Biggest win</p>
            <p className="text-sm text-vow-ink">{review.biggest_win}</p>
          </div>
        )}
        {review.biggest_setback && (
          <div className="bg-vow-bg p-5">
            <p className="vow-label mb-2">Biggest setback</p>
            <p className="text-sm text-vow-ink">{review.biggest_setback}</p>
          </div>
        )}
      </div>
      ...
      <div className="border-t border-vow-border pt-8">
        <p className="vow-label mb-4">Your coach</p>
        <div className="text-sm text-vow-ink whitespace-pre-wrap leading-relaxed">
      ...
      {patterns.length > 0 && (
        <div className="border-t border-vow-border pt-8">
          <p className="vow-label mb-4">Patterns detected ({patterns.length})</p>
          <div className="space-y-6">
            {patterns.map((p, i) => (
              <div key={i} className="border-b border-vow-border pb-6 last:border-0">
                <p className="text-xs text-vow-muted uppercase tracking-wide mb-2">{p.type.replace(/_/g, ' ')}</p>
                <p className="text-sm text-vow-ink font-medium mb-3">{p.description}</p>
                {p.evidence.length > 0 && (
                  <div className="mb-3">
                    <p className="text-xs text-vow-muted mb-1">Evidence</p>
                    <ul className="space-y-1">
                      {p.evidence.map((e, j) => (
                        <li key={j} className="text-xs text-vow-muted pl-3 border-l border-vow-border">
                          {e}
                        </li>
                      ))}
                    </ul>
                ...
                {p.hypothesis && (
                  <p className="text-xs text-vow-ink mb-2">
                    <span className="text-vow-muted">Hypothesis: </span>{p.hypothesis}
                  </p>
                )}
                {p.proposed_adjustment && (
                  <p className="text-xs text-vow-ink">
                    <span className="text-vow-muted">Suggestion: </span>{p.proposed_adjustment}
                  </p>
                )}
              </div>
            ))}
      ...
      {commitments.length > 0 && (
        <div className="border-t border-vow-border pt-8">
          <p className="vow-label mb-4">Proposed next week</p>
...
function ConfirmedReviewView({ review, pastReviews }: { review: Review; pastReviews: Review[] }) {
  return (
    <div>
      <PageHeader
        title="Weekly Review"
        subtitle={`${formatDate(review.week_start)} — ${formatDate(review.week_end)}`}
      />
      <div className="border-l-2 border-vow-success pl-4 mb-10">
        <p className="text-sm text-vow-ink font-medium">Review confirmed</p>
        <p className="text-xs text-vow-muted mt-0.5">Next week's commitments are locked in and sessions are scheduled.</p>
      </div>
...
function PastReviewsList({ reviews }: { reviews: Review[] }) {
  ...
  return (
    <div className="border-t border-vow-border pt-8 mt-10">
      <h3 className="vow-label mb-4">Past reviews</h3>
      <div className="border-t border-vow-border">
        {reviews.map((r) => (
          <div key={r.id} className="border-b border-vow-border">
            <button
              onClick={() => setExpanded(expanded === r.id ? null : r.id)}
              className="w-full flex items-center justify-between py-4 text-left hover:opacity-70 transition-opacity"
            >
              <div>
                <div className="text-sm text-vow-ink">{formatDate(r.week_start)} — {formatDate(r.week_end)}</div>
                <div className="text-xs text-vow-muted mt-0.5">
                  {r.completed_count}/{r.committed_count} sessions — {Math.round(r.completion_pct)}% — {r.status}
                </div>
              </div>
              <ArrowRight className={`w-4 h-4 text-vow-muted transition-transform ${expanded === r.id ? 'rotate-90' : ''}`} />
            </button>
            {expanded === r.id && (
              <div className="pb-8">
...
function MetricCell({ label, value }: { label: string; value: number }) {
  return (
    <div className="bg-vow-bg px-4 py-5 text-center">
      <div className="text-2xl vow-heading text-vow-ink">{value}</div>
