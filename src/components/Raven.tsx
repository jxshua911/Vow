...
export function RavenPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const [sessionsRes, goalsRes, awardsRes, snapshotsRes] = await Promise.all([
        ...
        supabase.from('raven_weekly_snapshots').select('*').eq('user_id', session.user.id).order('week_start', { ascending: false }).limit(2),
      ]);
      ...
      const storedSnapshots = snapshotsRes.data || [];
      const previous = storedSnapshots.length > 1 ? storedSnapshots[1].snapshot as RavenSnapshot : null;
  ...
  const trendMark = snapshot.trend === 'up' ? '↗' : snapshot.trend === 'down' ? '↘' : '→';
  ...
  return <div>
    ...
    <section className="border border-vow-border p-6 mb-8">
      <div className="flex items-start justify-between gap-6">
        <div><p className="vow-label mb-2">Raven score</p><div className="flex items-end gap-3"><span className="vow-heading text-5xl text-vow-ink">{snapshot.score}</span><span className="text-sm text-vow-muted mb-2">/ 100</span></div><div className="flex items-center gap-1 mt-3 text-xs text-vow-muted"><span aria-hidden="true">{trendMark}</span>{trendText}</div></div>
        <div className="text-right"><p className="vow-label mb-2">Current streak</p><div className="flex items-center justify-end gap-2"><span className="vow-heading text-3xl text-vow-ink">{snapshot.current_streak}</span><span className="text-xs text-vow-muted">days</span></div><p className="text-xs text-vow-muted mt-2">Best: {snapshot.best_streak} days</p></div>
      </div>
