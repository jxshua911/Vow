...
export function RavenReviewSection({ review }: { review?: Review | null }) {
  ...
  if (error) return <section className="border-t border-vow-border pt-8"><p className="vow-label mb-3">Turtle score</p><p className="text-sm text-vow-muted break-words">{error}</p></section>;
  ...
  return <section className="border-t border-vow-border pt-8 min-w-0 overflow-hidden">
    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6 mb-5">
      <div className="min-w-0">
        ...
        <p className="text-xs text-vow-muted mt-1 break-words">Slowly but surely, with VOW 🐢🤍</p>
        <div className="flex items-center gap-1 mt-2 text-xs text-vow-muted"><TrendIcon className="w-3.5 h-3.5 shrink-0" />{trendText}</div>
      </div>
    ...
    {snapshot.trend === 'down' && <div className="border-l-2 border-vow-ink pl-3 mb-5"><p className="text-sm text-vow-ink font-medium break-words">Your score went down. What’s happening, bro?</p><p className="text-xs text-vow-muted mt-1 break-words">The change is a signal to understand, not a reason to beat yourself up.</p></div>}
    {snapshot.trend === 'up' && <div className="border-l-2 border-vow-ink pl-3 mb-5"><p className="text-sm text-vow-ink font-medium break-words">Your consistency is moving up.</p><p className="text-xs text-vow-muted mt-1 break-words">Keep doing the work. The pattern is improving.</p></div>}
