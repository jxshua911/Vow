...
import { getNotificationPermission, getNotificationPreferences, requestNotificationPermission, setNotificationPreferences, syncUpcomingSessionNotifications, type NotificationPreferences } from '@/lib/notifications';
...
export function ProfilePage({ onLegal }: { onLegal?: () => void }) {
  ...
  const [selectedStyle, setSelectedStyle] = useState(ICON_STYLES[0].id);
  ...
  async function updateNotificationPref(key: keyof NotificationPreferences) {
    const next = { ...notificationPrefs, [key]: !notificationPrefs[key] };
    setNotificationPrefs(next);
    try { await setNotificationPreferences(next); if (session && notificationStatus === 'granted') { const { data } = await supabase.from('sessions').select('*').eq('user_id', session.user.id).eq('status', 'scheduled').gte('scheduled_at', new Date().toISOString()).order('scheduled_at', { ascending: true }); if (data) await syncUpcomingSessionNotifications(data); } } catch { setNotificationPrefs(notificationPrefs); }
  }
  ...
  return <div>
    ...
    <div className="border border-vow-border divide-y divide-vow-border">
      ...
      <div className="p-5"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="text-sm text-vow-ink">Notifications</p><p className="text-xs text-vow-muted mt-1">Turn on reminders for your scheduled VOW sessions.</p>{notificationStatus === 'denied' && <p className="text-xs text-vow-muted mt-2">Notifications are blocked. Enable them in your device settings, then return to VOW.</p>}{notificationStatus === 'unsupported' && <p className="text-xs text-vow-muted mt-2">Notifications are not available on this device.</p>}</div>{!notificationsEnabled && notificationStatus !== 'unsupported' && <button onClick={handleEnableNotifications} disabled={requesting} className="vow-btn-soft shrink-0 disabled:opacity-50">{requesting ? 'Enabling…' : 'Enable notifications'}</button>}</div>{notificationsEnabled && <div className="mt-5 border-t border-vow-border pt-4 space-y-3"><div className="flex items-center justify-between gap-4"><div><p className="text-xs text-vow-ink">Sound</p><p className="text-[11px] text-vow-muted mt-0.5">Play a sound with reminders.</p></div><button type="button" onClick={() => updateNotificationPref('sound')} aria-pressed={notificationPrefs.sound} className="vow-btn-soft shrink-0">{notificationPrefs.sound ? 'On' : 'Off'}</button></div><div className="flex items-center justify-between gap-4"><div><p className="text-xs text-vow-ink">Vibration</p><p className="text-[11px] text-vow-muted mt-0.5">Vibrate for reminders.</p></div><button type="button" onClick={() => updateNotificationPref('vibration')} aria-pressed={notificationPrefs.vibration} className="vow-btn-soft shrink-0">{notificationPrefs.vibration ? 'On' : 'Off'}</button></div></div>}</div>
