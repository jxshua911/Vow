...
const freeBenefits = ['1 active goal', 'Personalised plan with milestones', 'Scheduled sessions and progress tracking', 'A real taste of VOW AI planning'];
const premiumBenefits = ['Unlimited active goals', 'Adaptive plan rebuilding', 'Deeper AI coaching and planning', 'Goal-specific planning methodology', 'Advanced weekly review insights'];
    ...
    <section className="border border-vow-border bg-vow-bg rounded-2xl p-6 md:p-8 mb-8">
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-6">
        <div className="border border-vow-border rounded-xl p-5">
          ...
          <div className="space-y-2.5">{freeBenefits.map((benefit) => <div key={benefit} className="flex items-start gap-2 text-sm text-vow-muted"><Check className="w-4 h-4 mt-0.5 shrink-0 text-vow-ink" /><span>{benefit}</span></div>)}</div>
        </div>
        ...
        <div className="border border-vow-ink rounded-xl p-5 relative">
          ...
          <div className="space-y-2.5">{premiumBenefits.map((benefit) => <div key={benefit} className="flex items-start gap-2 text-sm text-vow-ink"><Check className="w-4 h-4 mt-0.5 shrink-0" /><span>{benefit}</span></div>)}</div>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-2 p-1 border border-vow-border rounded-xl mb-4" role="group" aria-label="Billing interval">
        <button type="button" onClick={() => setBilling('monthly')} className={`rounded-lg px-4 py-3 text-sm transition-colors ${billing === 'monthly' ? 'bg-vow-ink text-vow-bg' : 'text-vow-muted hover:text-vow-ink'}`} aria-pressed={billing === 'monthly'}><span className="block font-medium">Monthly</span><span className={`block text-xs mt-0.5 ${billing === 'monthly' ? 'opacity-80' : 'text-vow-muted'}`}>$5 / month</span></button>
        <button type="button" onClick={() => setBilling('yearly')} className={`relative rounded-lg px-4 py-3 text-sm transition-colors ${billing === 'yearly' ? 'bg-vow-ink text-vow-bg' : 'text-vow-muted hover:text-vow-ink'}`} aria-pressed={billing === 'yearly'}><span className="block font-medium">Yearly</span><span className={`block text-xs mt-0.5 ${billing === 'yearly' ? 'opacity-80' : 'text-vow-muted'}`}>$48 / year</span><span className="absolute -top-2 right-2 rounded-full border border-vow-border bg-vow-bg px-2 py-0.5 text-[10px] tracking-wide text-vow-muted">Save 20%</span></button>
      </div>

      <div className="relative mt-5">
        <div className="absolute inset-0 z-10 bg-vow-bg/55 backdrop-blur-[1px] rounded-xl pointer-events-auto" aria-hidden="true" />
        <button type="button" disabled aria-disabled="true" className="w-full rounded-xl border border-vow-border bg-vow-ink text-vow-bg px-4 py-3.5 flex items-center justify-center gap-2 opacity-50 cursor-not-allowed"><Lock className="w-4 h-4" /> Premium checkout coming soon</button>
      </div>
