...
function durationLabel(weeks: number) { if (weeks === 1) return '1 week'; if (weeks < 4) return `${weeks} weeks`; if (weeks === 4) return '4 weeks'; if (weeks % 4 === 0) return `${weeks / 4} months`; return `${weeks} weeks`; }
...
export function GoalPlanner({ userId, onCreated, onCancel }: { userId: string; onCreated: () => void; onCancel: () => void }) {
  ...
  async function loadCalendarContext() {
    try { const { data: status } = await supabase.functions.invoke('google-calendar-auth', { body: { action: 'status' } }); if (!status?.connected) return []; const { data } = await supabase.functions.invoke('google-calendar-events', { body: { action: 'list' } }); return Array.isArray(data?.events) ? data.events.slice(0, 30) : []; } catch { return []; }
  }
  ...
  async function buildPlan() {
    ...
    try {
      ...
      const { data, error: invokeError } = await supabase.functions.invoke('vow-goal-ai', { body: { mode: 'goal-plan', goal: { id: goalId, title: rawInput.trim(), why_it_matters: whyItMatters.trim() || null, horizon: `${durationWeeks} weeks` }, message: `Follow-up answers:\n${clarification.questions.map((question, index) => `Q: ${question}\nA: ${cleanAnswers[index] || 'Skipped / not supplied'}`).join('\n')}\n\nSuitable days: ${availableDays.join(', ')}\n\nRequested duration: ${durationWeeks} weeks.\nAI recommendation: ${clarification.recommended_duration_weeks} weeks.\nThe user's requested duration is authoritative. Generate exactly a ${durationWeeks}-week plan. Use only the suitable days where possible. Keep schedule concise and actionable.`, scope: 'goal-plan-generation', calendar, references } });
      if (invokeError || !data?.structured) throw new Error((data?.error as string) || 'VOW AI could not build the plan.');
