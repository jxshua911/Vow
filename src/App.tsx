...
function AppContent() {
  ...
  useEffect(() => {
    ...
    supabase.from('user_settings').select('*').eq('user_id', session.user.id).maybeSingle().then(({ data }) => { setSettings(data as UserSettings | null); setSettingsLoading(false); });
  }, [session]);
  ...
  useEffect(() => {
    if (splashMinElapsed && contentReady && !splashFadingOut) { setSplashFadingOut(true); const timer = window.setTimeout(() => setSplashMounted(false), SPLASH_FADE_OUT_MS); return () => window.clearTimeout(timer); }
