...
function extractTarget(text: string, goalType: string): string | null {
  const patterns = [
    /(\d+(?:\.\d+)?)\s*(km|kilometres?|kilometers?|miles?|mi|pages?|books?|hours?|hrs?|minutes?|mins?|sessions?)/i,
    /(?:save|saving|budget)\s+(?:of\s+)?([$£€]?\s*\d+(?:[,.]\d+)?)/i,
    /(?:under|below|within|in)\s+(\d+(?:\.\d+)?)\s*(hours?|hrs?|minutes?|mins?|days?|weeks?|months?)/i,
  ];
  for (const pattern of patterns) {
    const match = text.match(pattern);
    if (match) return match[0].trim();
  }
  if (/\b(?:5k|10k|half marathon|marathon)\b/i.test(text)) return text.match(/\b(?:5k|10k|half marathon|marathon)\b/i)?.[0] || null;
...
function extractTimeTarget(text: string): string | null {
  const match = text.match(/\b(?:under|below|within)\s+(\d+(?:\.\d+)?)\s*(hours?|hrs?|minutes?|mins?)\b/i);
  return match?.[0]?.trim() || null;
}

function directionFor(text: string): ArmadilloResult['direction'] {
  if (/\b(?:reduce|decrease|lower|cut|spend less|lose)\b/i.test(text)) return 'decrease';
...
export function analyseGoalForEvidence(input: { title?: string | null; outcome?: string | null; why_it_matters?: string | null }): ArmadilloResult {
  ...
  const base = match || {
    category: 'General', goal_type: 'Goal', metric: 'measurable progress toward the stated outcome',
    evidence: ['manual progress updates', 'goal milestones', 'completed sessions or actions'], integration: null,
    planning_strategy: 'define the clearest measurable outcome, then break it into small repeatable actions', keywords: []
  };
  const target = extractTarget(text, base.goal_type);
  const timeTarget = extractTimeTarget(text);
  const vague = /\b(?:get better|improve|do more|be better|become better|get fit|work on|try to)\b/i.test(text);
  const needsClarification = !match || (vague && !target);
  const clarificationReasons = !match
    ? ['The goal does not yet map cleanly to a specific measurable goal type.']
    : vague && !target ? ['The intended outcome is clear enough to start, but the measurable target is still ambiguous.'] : [];
  return {
    ...
    secondary_metric: base.goal_type === 'Running' && timeTarget ? 'time' : null,
