...
function accountKey(preferredAccountEmail?: string) {
  return (preferredAccountEmail || 'phone').trim().toLowerCase().replace(/[^a-z0-9@._+-]/g, '_');
}
function isSynced(sessionId: string, preferredAccountEmail?: string): boolean {
  return localStorage.getItem(`${SYNCED_PREFIX}${accountKey(preferredAccountEmail)}:${sessionId}`) === 'true';
}
function markSynced(sessionId: string, preferredAccountEmail?: string): void {
  localStorage.setItem(`${SYNCED_PREFIX}${accountKey(preferredAccountEmail)}:${sessionId}`, 'true');
}
...
async function getCalendarId(preferredAccountEmail?: string): Promise<string | undefined> {
  ...
  if (preferredAccountEmail) {
    ...
    const existing = writable.find((calendar) => {
  ...
  const existingVowCalendar = writable.find((calendar) => calendar.title === 'VOW');
  if (existingVowCalendar?.id) return existingVowCalendar.id;
  const created = await CapacitorCalendar.createCalendar({ title: 'VOW', color: '#D4AF37' });
  return created.id;
}
...
export async function addSessionToNativeCalendar(session: Session, preferredAccountEmail?: string): Promise<string | null> {
  ...
  if (!calendarId) throw new Error('No writable phone calendar was found.');
