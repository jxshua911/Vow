...
function dateOnly(value: string) { return new Date(`${value.slice(0, 10)}T00:00:00`); }
function isoDate(date: Date) { return date.toISOString().slice(0, 10); }
function monday(date: Date) { const d = new Date(date); d.setHours(0, 0, 0, 0); const day = d.getDay(); d.setDate(d.getDate() + (day === 0 ? -6 : 1 - day)); return d; }
function weekEnd(start: Date) { const d = new Date(start); d.setDate(d.getDate() + 6); return d; }
...
export function buildRavenWeeks(sessions: Session[], now = new Date()): RavenWeek[] {
  ...
  for (const session of relevant) {
    ...
    const current = buckets.get(key) || { week_start: key, week_end: isoDate(weekEnd(start)), committed: 0, completed: 0, skipped: 0, moved: 0, completion_pct: 0 };
    current.committed += 1;
  ...
  return [...buckets.values()].map((week) => ({ ...week, completion_pct: week.committed ? Math.round((week.completed / week.committed) * 100) : 0 })).sort((a, b) => a.week_start.localeCompare(b.week_start));
}

export function calculateStreaks(sessions: Session[], now = new Date()) {
  const completedDays = new Set(sessions.filter((s) => s.status === 'completed' && new Date(s.scheduled_at) <= now).map((s) => isoDate(new Date(s.scheduled_at))));
  ...
  for (let i = 1; i < days.length; i += 1) {
    ...
    if (diff === 1) { run += 1; best = Math.max(best, run); } else run = 1;
  }
...
export function calculateRavenSnapshot(sessions: Session[], previousSnapshot?: RavenSnapshot | null, now = new Date()): RavenSnapshot {
  ...
  return { score, previous_score: previousScore, score_delta: delta, trend, current_streak: streaks.current, best_streak: streaks.best, total_completed: completed, total_sessions: observed, completion_pct: observed ? Math.round((completed / observed) * 100) : 0, best_weekly_completion_pct: bestWeek?.completion_pct || 0, best_week_start: bestWeek?.week_start || null, weekly_completed_best: weeks.reduce((best, week) => Math.max(best, week.completed), 0), weeks_observed: weeks.length, recent_weeks: recent, signals };
}
...
export function getRavenAwards(snapshot: RavenSnapshot, previousAwards: RavenAward[] = [], now = new Date()): RavenAward[] {
  ...
  const add = (key: string, title: string, description: string) => { if (!existing.has(key)) awards.push({ key, title, description, earned_at: now.toISOString() }); };
...
export function goalProgress(sessions: Session[], goals: Goal[]) {
  return goals.map((goal) => { const items = sessions.filter((s) => s.goal_id === goal.id); const completed = items.filter((s) => s.status === 'completed').length; return { goal, completed, total: items.length, pct: items.length ? Math.round((completed / items.length) * 100) : 0 }; });
}
