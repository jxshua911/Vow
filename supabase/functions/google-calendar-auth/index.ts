...
serve(async (req) => {
  ...
  try {
    if (req.method === "POST") {
      ...
      if (body.action === "status" && !configured) return json({ connected: false, available: false, configured: false });
      ...
      if (body.action === "status") {
        const { data: connection, error } = await admin.from("google_calendar_connections").select("id, access_token").eq("user_id", user.id).maybeSingle();
      ...
      authorizationUrl.search = new URLSearchParams({ client_id: clientId!, redirect_uri: googleRedirectUri!, response_type: "code", access_type: "offline", prompt: "consent", state, scope: "https://www.googleapis.com/auth/calendar" }).toString();
    ...
    const { error: saveError } = await admin.from("google_calendar_connections").upsert({ user_id: connection.user_id, access_token: tokenData.access_token, refresh_token: tokenData.refresh_token ?? null, expires_at: expiresAt, scope: tokenData.scope ?? "https://www.googleapis.com/auth/calendar", oauth_state: null, oauth_state_expires_at: null, oauth_redirect_uri: null, updated_at: new Date().toISOString() }, { onConflict: "user_id" });
    if (saveError) throw saveError;
