...
Deno.serve(async req=>{
  ...
  try{
    ...
    const input=rows.map((r:any)=>[r.domain,r.topic,r.title,r.content,JSON.stringify(r.principles||[]),JSON.stringify(r.recommended_actions||[]),JSON.stringify(r.metrics||[]),JSON.stringify(r.cautions||[])].join("\n"));
    ...
    for(let i=0;i<rows.length;i++){const vector=payload?.data?.[i]?.embedding;if(!Array.isArray(vector)||vector.length!==384)throw new Error("EMBEDDING_DIMENSION_MISMATCH");const {error:e}=await db.from("vow_knowledge").update({embedding:vector}).eq("id",rows[i].id);if(e)throw e;}
    ...
    return json({ok:true,embedded:rows.length,remaining:count||0,model:"text-embedding-3-small",dimensions:384});
