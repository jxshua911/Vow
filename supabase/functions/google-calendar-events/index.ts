...
serve(async (req) => {
  ...
  try {
    ...
    try { accessToken = await refreshAccessToken(connection as Record<string, unknown>, admin) as string; } catch (error) { if (error instanceof Error && 'code' in error) return json({ error: error.message, code: (error as Error & { code: string }).code }, 401); throw error; }
    ...
    if (action === "list") {
      ...
      for (const calendar of calendars) {
        ...
        const path = `/calendars/${encodeURIComponent(calendarId)}/events?timeMin=${encodeURIComponent(timeMin)}&timeMax=${encodeURIComponent(timeMax)}&singleEvents=true&orderBy=startTime&showDeleted=false&maxResults=2500`;
        try {
          const data = await googleGet(path, accessToken);
          for (const event of (Array.isArray(data.items) ? data.items : [])) allEvents.push({ ...(event as Record<string, unknown>), calendarId, calendarName: calendar.summary || calendarId });
        } catch (error) {
          console.warn('[VOW Calendar] Skipping unreadable calendar', calendarId, error);
        }
      }
      allEvents.sort((a, b) => {
        const getStart = (value: unknown) => { const item = value as { start?: { dateTime?: string; date?: string } }; return item.start?.dateTime || item.start?.date || ''; };
        return getStart(a).localeCompare(getStart(b));
      });
      return json({ events: allEvents, calendars: calendars.map((calendar: { id?: string; summary?: string; primary?: boolean; accessRole?: string }) => ({ id: calendar.id, summary: calendar.summary, primary: calendar.primary, accessRole: calendar.accessRole })) });
    }

    if (action === "create") {
      const event = body?.event;
      if (!event) return json({ error: "Missing event" }, 400);
      const response = await fetch(`${GOOGLE_CALENDAR_API}/calendars/primary/events`, { method: "POST", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify(event) });
      const data = await response.json();
      if (!response.ok) {
