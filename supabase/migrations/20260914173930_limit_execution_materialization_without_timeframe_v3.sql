...
begin
  ...
  if exists (select 1 from public.sessions where goal_id = new.id) or exists (select 1 from public.goal_plan_items where goal_id = new.id) then return new; end if;
  ...
  if v_match is not null then
    v_seconds := v_match[1]::numeric * case v_match[2] when 'minute' then 60 when 'minutes' then 60 when 'hour' then 3600 when 'hours' then 3600 when 'day' then 86400 when 'days' then 86400 when 'week' then 604800 when 'weeks' then 604800 when 'month' then 2592000 when 'months' then 2592000 else 0 end;
  end if;
  v_count := case when v_seconds > 0 then least(jsonb_array_length(v_steps), 24) else 1 end;
