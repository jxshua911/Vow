...
CREATE POLICY "Users can read their own VOW entitlement" ON public.vow_user_entitlements FOR SELECT TO authenticated USING (auth.uid() = user_id);
...
CREATE INDEX IF NOT EXISTS vow_entitlement_usage_user_feature_period_idx ON public.vow_entitlement_usage(user_id, feature, period_start, created_at DESC);
...
CREATE POLICY "Users can read their own VOW entitlement usage" ON public.vow_entitlement_usage FOR SELECT TO authenticated USING (auth.uid() = user_id);

CREATE OR REPLACE FUNCTION public.vow_is_premium(p_user_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM public.vow_user_entitlements WHERE user_id = p_user_id AND plan = 'premium' AND status IN ('active','trialing') AND (current_period_end IS NULL OR current_period_end > now()));
$$;

CREATE OR REPLACE FUNCTION public.vow_get_entitlement_snapshot()
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
DECLARE uid uuid := auth.uid(); premium boolean; period date := date_trunc('month', now())::date; active_goals integer; planning_used integer; replan_used integer; review_used integer;
BEGIN
  ...
  RETURN jsonb_build_object('plan', CASE WHEN premium THEN 'premium' ELSE 'free' END, 'active_goals', active_goals, 'active_goal_limit', CASE WHEN premium THEN NULL ELSE 1 END, 'planning_used', planning_used, 'planning_limit', CASE WHEN premium THEN NULL ELSE 10 END, 'adaptive_replans_used', replan_used, 'adaptive_replans_limit', CASE WHEN premium THEN NULL ELSE 1 END, 'advanced_reviews_used', review_used, 'advanced_reviews_limit', CASE WHEN premium THEN NULL ELSE 1 END, 'period_start', period);
END; $$;
...
CREATE OR REPLACE FUNCTION public.vow_guard_review_entitlement()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE result jsonb;
BEGIN
  result := public.vow_consume_entitlement('advanced_review', jsonb_build_object('review_id', NEW.id, 'week_start', NEW.week_start));
  IF coalesce((result->>'allowed')::boolean, false) = false THEN
    RAISE EXCEPTION 'VOW_PREMIUM_ADVANCED_REVIEW_REQUIRED';
  END IF;
  RETURN NEW;
END; $$;

DROP TRIGGER IF EXISTS vow_review_entitlement_guard ON public.reviews;
CREATE TRIGGER vow_review_entitlement_guard BEFORE INSERT ON public.reviews FOR EACH ROW EXECUTE FUNCTION public.vow_guard_review_entitlement();
