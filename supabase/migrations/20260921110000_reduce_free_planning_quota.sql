-- Tighten the Free planning allowance to 3 AI planning actions per calendar month.
-- Premium removes this product quota; infrastructure rate limits still apply to every plan.
...
BEGIN
  ...
  SELECT count(*)::integer INTO active_goals
  FROM public.goals
  WHERE user_id = uid AND status IN ('active','locked');

  SELECT count(*)::integer INTO planning_used
  FROM public.vow_entitlement_usage
  WHERE user_id = uid AND feature = 'planning_action' AND period_start = period;

  SELECT count(*)::integer INTO replan_used
  FROM public.vow_entitlement_usage
  WHERE user_id = uid AND feature = 'adaptive_replan' AND period_start = period;

  SELECT count(*)::integer INTO review_used
  FROM public.vow_entitlement_usage
  WHERE user_id = uid AND feature = 'advanced_review' AND period_start = period;
  ...
  RETURN jsonb_build_object(
    ...
    'planning_limit', CASE WHEN premium THEN NULL ELSE 3 END,
