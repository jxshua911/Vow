...
-- Keeps the planner's existing structured knowledge fields intact while
-- improving multi-intent routing and preventing topic-family contamination.

create or replace function public.match_vow_knowledge_keyword(
  query_text text,
...
with raw as (
  ...
  select k.*, c.q, c.topics, c.inferred_domain,
    (case when c.inferred_domain is not null and lower(k.domain) = c.inferred_domain then 8 else 0 end
     + case when lower(k.topic) = any(c.topics) then 25 else 0 end
     + case when exists(select 1 from unnest(c.topics) t where lower(k.topic) like t || '%') then 8 else 0 end
     + case when lower(k.title || ' ' || k.content) like '%' || c.q || '%' and length(c.q) > 8 then 2 else 0 end
    )::real as score
  from public.vow_knowledge k cross join classified c
  where k.active = true
    and (domain_filter is null or lower(k.domain) = lower(domain_filter))
    and (domain_filter is not null or c.inferred_domain is null or lower(k.domain) = c.inferred_domain)
), allowed as (
  select * from candidates c
  where c.inferred_domain is null
     or lower(c.topic) = any(c.topics)
     or exists(select 1 from unnest(c.topics) t where lower(c.topic) like t || '%')
     or (c.inferred_domain = 'career' and lower(c.topic) in ('job_search','cv_resume','portfolio','engineering','professional_skills'))
     or (c.inferred_domain = 'education' and lower(c.topic) in ('mathematics fundamentals','mathematics','exam_preparation','exams','revision','study','study_skills'))
     or (c.inferred_domain = 'creative' and lower(c.topic) in ('writing','story structure','finishing work','music','guitar','photography fundamentals','photography'))
     or (c.inferred_domain = 'travel' and lower(c.topic) in ('trip planning','travel planning','itinerary','planning','cultural learning'))
)
select id, domain, topic, title, content, principles, recommended_actions, metrics, cautions, source_url, score
from allowed
where score > 0
