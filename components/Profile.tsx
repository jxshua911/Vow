...
const ICON_COLOURS = [
  { name: 'White', value: 'white', fg: '#FFFFFF', bg: '#000000' },
  { name: 'Black', value: 'black', fg: '#000000', bg: '#FFFFFF' },
...
export function ProfilePage({ onLegal }: { onLegal?: () => void }) {
  ...
  const [selected, setSelected] = useState<IconColour>('white');
        ...
        <div className="max-w-xl mx-auto space-y-6">
          ...
          <section className="rounded-2xl border border-current/10 p-5 space-y-5">
            <div>
              ...
              <p className="text-sm opacity-60 mt-1">Choose your VOW icon colour.</p>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {ICON_COLOURS.map((option) => (
                <button key={option.value} type="button" onClick={() => setSelected(option.value)} aria-pressed={selected === option.value} className={`rounded-2xl border p-3 flex flex-col items-center gap-2 ${selected === option.value ? 'border-current ring-2 ring-current/20' : 'border-current/10'}`}>
      ...
      <div className="max-w-xl mx-auto space-y-6">
        ...
        <section className="rounded-2xl border border-current/10 overflow-hidden">
          <button type="button" onClick={() => setCustomiseOpen(true)} className="w-full px-5 py-4 flex items-center justify-between text-left hover:bg-current/5 transition-colors">
            <div>
              <div className="text-base font-medium">Customise</div>
              <div className="text-sm opacity-60 mt-1">Personalise your VOW experience</div>
            </div>
